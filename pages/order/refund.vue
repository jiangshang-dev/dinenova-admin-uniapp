...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.orderSn || ('售后#' + item.id)
    },
    statusOf(item) {
      return { label: item.status || '售后', tone: 'warn' }
    },
    linesOf(item) {
      return ['退款 ¥' + money(item.amount || item.refundAmount), item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getRefundList(this.buildQuery())
