...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.name || ('卡券#' + item.id)
    },
    statusOf(item) {
      return statusAN(item.status)
    },
    linesOf(item) {
      return ['面额 ' + (item.amount != null ? money(item.amount) : '-'), '类型 ' + (item.type || '-'), item.groupName || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getCouponList(this.buildQuery())
