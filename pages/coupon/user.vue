...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.couponName || item.name || ('持券#' + item.id)
    },
    statusOf(item) {
      return statusAN(item.status)
    },
    linesOf(item) {
      return ['会员 ' + (item.userName || item.mobile || '-'), item.code || '', item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getUserCouponList(this.buildQuery())
