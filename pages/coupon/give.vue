...
import { getGiveList } from '@/api/coupon'
...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.couponName || ('转赠#' + item.id)
    },
    statusOf(item) {
      return { label: '转赠', tone: 'warn' }
    },
    linesOf(item) {
      return ['从 ' + (item.userName || '-') + ' → ' + (item.giveUserName || '-'), item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getGiveList(this.buildQuery())
