...
import { getSendLogList } from '@/api/coupon'
...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.couponName || ('发券#' + item.id)
    },
    statusOf(item) {
      return { label: '已发放', tone: 'primary' }
    },
    linesOf(item) {
      return ['会员 ' + (item.userName || item.mobile || '-'), '数量 ' + (item.num || 1), item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getSendLogList(this.buildQuery())
