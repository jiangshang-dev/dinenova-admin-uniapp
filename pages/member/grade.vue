...
import { getUserGradeList } from '@/api/userGrade'
...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.name || ('等级#' + item.id)
    },
    statusOf(item) {
      return statusAN(item.status)
    },
    linesOf(item) {
      return ['折扣 ' + (item.discount || '-') , '升级条件 ' + (item.catchValue || '-')]
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getUserGradeList(this.buildQuery())
