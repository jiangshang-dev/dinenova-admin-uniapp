...
import { getOpenGiftList } from '@/api/openGift'
...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.name || item.couponName || ('赠礼#' + item.id)
    },
    statusOf(item) {
      return statusAN(item.status)
    },
    linesOf(item) {
      return ['数量 ' + (item.num || item.amount || '-'), item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getOpenGiftList(this.buildQuery())
