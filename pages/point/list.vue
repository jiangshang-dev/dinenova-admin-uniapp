...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.userName || item.mobile || ('积分#' + item.id)
    },
    statusOf(item) {
      return { label: (item.amount > 0 ? '+' : '') + (item.amount || 0), tone: item.amount > 0 ? 'success' : 'danger' }
    },
    linesOf(item) {
      return [item.description || item.remark || '', item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getPointList(this.buildQuery())
