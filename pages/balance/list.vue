...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.userName || item.mobile || ('余额#' + item.id)
    },
    statusOf(item) {
      return { label: '¥' + money(item.amount), tone: item.amount > 0 ? 'success' : 'danger' }
    },
    linesOf(item) {
...
{}
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getBalanceList(this.buildQuery())
