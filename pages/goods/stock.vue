...
export default {
  ...
  methods: {
    titleOf(item) {
      return item.goodsName || item.name || ('库存#' + item.id)
    },
    statusOf(item) {
      return statusAN(item.status)
    },
    linesOf(item) {
      return ['数量 ' + (item.num != null ? item.num : item.quantity || '-'), '类型 ' + (item.type || '-'), item.createTime || '']
    },
    onItem(item) {
{}
    },
    ...
    async fetchList() {
      ...
      this.loading = true
      try {
        const res = await getStockList(this.buildQuery())
